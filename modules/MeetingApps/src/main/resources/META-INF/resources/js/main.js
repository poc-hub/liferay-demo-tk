...
function addFunction() {
	...
	var res = '<tr>'
			...
			+ '<input type="date" id="expectedDueDate" name="ExpectedDueDate" class="form-control" required/>'
			+ '</div>'
			+ '</div>'
			+ '</td>'
			+ '<td>'
			...
			+ '<input type="text" id="persionInCharge" name="PersionInCharge" class="form-control" required/>'
			+ '</div>' + '</div>' +
...
function addRow() {
	...
	for (var c = 0; c < arrHead.length; c++) {
		...
		if (c == 6) {      // the last column.
			...
			var html = "<button type='button' class='btn' id='remove'><i class='fa fa-minus' aria-hidden='true'></i></button>"
		...
		// Add catagory dropdown
			...
			ExpectedDueDate.setAttribute('required', '');
			var label = document.createElement("label");
